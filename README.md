...
Проект построен на базе **Senior-level** паттернов и технологий:
...
└── architecture.txt    # Схема архитектуры в формате PlantUML
```
...
1.  **Умный экспорт**: Генерация Excel-отчетов на Senior-уровне с использованием стриминга (`StreamWriter`), что позволяет выгружать миллионы строк без утечек памяти.
