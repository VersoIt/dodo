...
const openConfirm = (order: any, nextStatus: string) => {
  ...
  pendingAction.value = {
    ...
    status: nextStatus,
    title: isStart ? 'Pick Up Order?' : 'Deliver Order?',
    description: isStart 
      ? `Accept Order #${order.order_number.split('-').pop()} for delivery?` 
...
const handleConfirm = async () => {
  ...
  try {
    ...
    addToast(status === STATUS_DELIVERING ? 'Order picked up' : 'Order delivered!', 'success')
  ...
  <div class="max-w-6xl mx-auto px-4 py-8">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
      <div class="flex items-center gap-4">
        ...
        <div>
          <h1 class="text-4xl font-black tracking-tighter uppercase italic text-secondary">Logistics</h1>
          <p class="text-base-content/50 font-bold uppercase text-[10px] tracking-[0.2em]">Live Deliveries</p>
        </div>
      </div>
      <div class="flex items-center gap-4 bg-base-100 border border-base-200 p-2 rounded-2xl shadow-sm">
        ...
        <div class="pr-4">
          <p class="text-[10px] font-black uppercase opacity-30 leading-none mb-1">On Duty</p>
    ...
    <div v-else-if="filteredOrders.length === 0" class="text-center py-40 bg-base-100 rounded-[3.5rem] border-2 border-dashed border-base-300">
      ...
      <h2 class="text-2xl font-black opacity-20 uppercase tracking-tighter">No deliveries</h2>
      <p class="text-base-content/30 max-w-xs mx-auto mt-2 font-bold uppercase text-[10px] tracking-widest">Wait for ready orders</p>
    </div>
      ...
      >
        <div class="bg-secondary/5 px-8 py-4 flex justify-between items-center border-b border-secondary/10">
           ...
           <div class="badge font-black uppercase text-[10px] py-3.5 px-4 rounded-lg border-none" 
            :class="order.status === STATUS_READY ? 'badge-success' : 'badge-info'">
            {{ order.status }}
           </div>
        </div>
        ...
        <div class="card-body p-8">
          <div class="flex items-start gap-6 mb-8">
            ...
            <div class="flex-1">
              <p class="text-[10px] font-black uppercase tracking-widest text-base-content/30 mb-2">Delivery Address</p>
              ...
              <div v-if="order.address.house" class="flex gap-3 mt-5">
                <div class="bg-base-200 px-4 py-2 rounded-xl text-sm font-black">House: {{ order.address.house }}</div>
                <div v-if="order.address.apartment" class="bg-base-200 px-4 py-2 rounded-xl text-sm font-black">Apt: {{ order.address.apartment }}</div>
              </div>
            </div>
          </div>
          ...
          <div class="py-8">
            <p class="text-[10px] font-black uppercase tracking-widest text-base-content/30 mb-4">Items</p>
          ...
          <div class="card-actions">
            ...
            >
              <Truck class="w-6 h-6" /> Take Order
            </button>
            <button 
            ...
            >
              <CheckCircle2 class="w-6 h-6" /> Delivered
            </button>
          </div>
        </div>
      </div>
      ...
      <div v-if="showConfirmModal" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
        ...
        <Transition name="modal-zoom" appear>
          <div class="relative bg-base-100 w-full max-w-sm rounded-[3rem] shadow-2xl border border-base-200 p-10 text-center">
              <div class="bg-secondary/10 w-24 h-24 rounded-full flex items-center justify-center mx-auto mb-8">
              ...
              <p class="text-base-content/50 text-sm mb-10 leading-relaxed font-medium">
                {{ pendingAction?.description }}
              </p>
              <div class="flex flex-col gap-3">
                <button @click="handleConfirm" class="btn btn-secondary btn-lg rounded-2xl h-16 font-black uppercase shadow-xl shadow-secondary/20">
                  Confirm
                </button>
