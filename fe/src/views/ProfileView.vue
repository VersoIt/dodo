...
const fetchOrders = async () => {
  ...
  } catch (err) { console.error('Failed to fetch orders:', err) } finally { loadingOrders.value = false }
}
  ...
  <div class="max-w-4xl mx-auto py-8 px-4">
    <div class="flex flex-col md:flex-row gap-8">
      <div class="w-full md:w-64 space-y-2">
        <button @click="activeTab = 'profile'" class="btn btn-ghost w-full justify-start gap-3" :class="{ 'btn-active bg-primary/10 text-primary': activeTab === 'profile' }"><User class="w-5 h-5" />Информация</button>
        <button v-if="authStore.user?.role === 'client'" @click="activeTab = 'orders'; fetchOrders()" class="btn btn-ghost w-full justify-start gap-3" :class="{ 'btn-active bg-primary/10 text-primary': activeTab === 'orders' }"><Package class="w-5 h-5" />Мои заказы</button>
        <div class="divider"></div>
        <button @click="authStore.logout(); $router.push('/login')" class="btn btn-ghost w-full justify-start gap-3 text-error hover:bg-error/10">Выйти</button>
      </div>
      ...
      <div class="flex-1">
        ...
        <div v-if="activeTab === 'orders'" class="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
          ...
          <div v-if="loadingOrders" class="flex justify-center py-12"><span class="loading loading-spinner loading-lg text-primary"></span></div>
          <div v-else-if="orders.length === 0" class="card bg-base-100 border border-dashed border-base-300 py-12"><div class="flex flex-col items-center text-center"><Package class="w-16 h-16 text-base-content/20 mb-4" /><h3 class="text-xl font-bold">Заказов пока нет</h3><p class="text-base-content/60 mb-6">Проголодались? Пора заказать вкусную пиццу!</p><router-link to="/" class="btn btn-primary">В меню</router-link></div></div>
          <div v-else v-for="order in orders" :key="order.order_id" class="card bg-base-100 shadow-md border border-base-200 hover:border-primary/30 transition-all hover:shadow-lg"><div class="card-body p-6 flex flex-row justify-between items-center"><div><div class="flex items-center gap-3 mb-1"><span class="font-mono font-bold text-lg tracking-tight">#{{ order.order_number }}</span><div class="badge badge-outline uppercase text-[10px] font-black tracking-widest" :class="{ 'badge-success border-green-500 text-green-500': order.status === 'completed' || order.status === 'paid', 'badge-warning border-yellow-500 text-yellow-500': order.status === 'cooking' || order.status === 'created' }">{{ order.status }}</div></div><p class="text-[10px] uppercase font-bold text-base-content/40">15 Февраля, 2026</p></div><div class="text-right"><p class="text-2xl font-black text-primary">{{ formatPrice(order.final_price) }}</p><router-link :to="'/order/' + order.order_id" class="btn btn-link btn-xs text-primary no-underline hover:underline p-0 h-auto min-h-0">Подробнее</router-link></div></div></div>
        </div>
      </div>
    </div>
  </div>
