...
<template>
  <div class="max-w-5xl mx-auto pb-20">
    <h1 class="text-5xl font-black mb-10 tracking-tight italic uppercase">Ваша корзина</h1>
    ...
    <div v-if="cartStore.items.length === 0" class="card bg-base-100 shadow-xl p-20 text-center border border-base-200 rounded-[3.5rem]">
      <div class="flex flex-col items-center gap-6">
        ...
        <p class="text-base-content/50 max-w-sm font-medium">Похоже, вы еще не выбрали свою идеальную пиццу. Самое время это исправить!</p>
      ...
      <div class="lg:col-span-8 space-y-10">
          ...
          <div v-for="item in cartStore.items" :key="item.id" class="card card-side bg-base-100 shadow-sm border border-base-200 p-5 transition-all hover:shadow-lg rounded-3xl overflow-hidden group">
            ...
            <div class="card-body py-0 px-6 justify-between">
              <div class="flex justify-between items-start">
                ...
                <button @click="cartStore.removeFromCart(item.id)" class="btn btn-ghost btn-sm btn-circle text-error/30 hover:text-error hover:bg-error/10 transition-all"><Trash2 class="w-5 h-5" /></button>
              </div>
        ...
        <div class="card bg-base-100 shadow-xl border border-base-200 overflow-hidden rounded-[2.5rem]">
          <div class="bg-base-200/50 px-10 py-5 border-b border-base-200 flex items-center justify-between">
            ...
            <span class="badge badge-primary badge-sm font-bold uppercase text-[8px] tracking-widest px-3">Required</span>
          </div>
          <div class="card-body p-10 space-y-8">
            <div class="form-control w-full">
              ...
              <div class="relative"><MapPin class="absolute left-4 top-3.5 w-5 h-5 opacity-20" />
                <input v-model="address.street" type="text" placeholder="пр. Ленина" class="input input-bordered w-full pl-12 rounded-2xl h-12 font-bold" required />
              </div>
            </div>
            <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
              <div class="form-control">
                <label class="label"><span class="label-text font-black uppercase text-[10px] opacity-40">Дом</span></label>
                <div class="relative"><Building2 class="absolute left-4 top-3.5 w-5 h-5 opacity-20" />
                  <input v-model="address.house" type="text" class="input input-bordered w-full pl-12 rounded-2xl h-12 font-bold" />
                </div>
              </div>
              <div class="form-control">
                <label class="label"><span class="label-text font-black uppercase text-[10px] opacity-40">Кв/Офис</span></label>
                <input v-model="address.apartment" type="text" class="input input-bordered w-full px-6 rounded-2xl h-12 font-bold" />
              </div>
              <div class="form-control">
                <label class="label"><span class="label-text font-black uppercase text-[10px] opacity-40">Этаж</span></label>
                <input v-model="address.floor" type="text" class="input input-bordered w-full px-6 rounded-2xl h-12 font-bold" />
              </div>
              <div class="form-control">
                <label class="label"><span class="label-text font-black uppercase text-[10px] opacity-40">Подъезд</span></label>
                <input v-model="address.floor" type="text" class="input input-bordered w-full px-6 rounded-2xl h-12 font-bold" />
              </div>
            </div>
            <div class="form-control">
              <label class="label"><span class="label-text font-black uppercase text-[10px] opacity-40">Комментарий</span></label>
              <div class="relative"><MessageSquare class="absolute left-4 top-4 w-5 h-5 opacity-20" />
                <textarea v-model="address.comment" class="textarea textarea-bordered w-full pl-12 rounded-2xl min-h-[120px] pt-4 font-bold"></textarea>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="card bg-base-100 shadow-2xl border border-primary/10 sticky top-24 overflow-hidden rounded-[2.5rem]">
          ...
          <div class="card-body p-8">
            ...
            <div class="card-actions mt-10">
              <button @click="handleCheckout" class="btn btn-primary btn-block h-20 rounded-3xl shadow-2xl shadow-primary/30 gap-4 font-black uppercase text-lg italic transition-all hover:scale-[1.02]" :disabled="isPlacingOrder || !isAddressValid">
                <template v-if="isPlacingOrder"><span class="loading loading-spinner"></span></template>
                <template v-else><Check class="w-6 h-6" /> Оформить заказ</template>
              </button>
      ...
      <div class="p-12 flex flex-col items-center">
        ...
        <div class="flex flex-col w-full gap-4">
          <button @click="confirmPayment" class="btn btn-primary btn-block h-20 rounded-3xl text-xl font-black uppercase shadow-2xl shadow-primary/30 tracking-tight transition-all hover:scale-[1.02]" :disabled="isPaying">
            ...
            <span v-else>Я оплатил</span>
          </button>
          <div class="flex items-center justify-center gap-3 mt-4 opacity-20"><ShieldCheck class="w-5 h-5" /><span class="text-[10px] font-black uppercase tracking-[0.3em]">Secure Transaction</span></div>
        </div>
      </div>
    </AppModal>
  </div>
